...
\[
  q=\min\{p:p\mid N\}
\]
be its smallest prime divisor.  This paper isolates the certified
minimal-prime part of a broader cyclotomic-certificate framework.  It proves, within
the frozen certificate release \(\Csmall\), that the branch inventories for
\[
  q=5,\ 7,\ 11,\ 13,\ 17
\]
are exhausted.  The result is deliberately not stated as an unconditional
...
nonexistence proof.  Nielsen proved that an odd perfect number must have many
distinct prime divisors and also developed effective upper-bound technology
\cite{Nielsen2007,Nielsen2015}.  Ochem and Rao proved the landmark lower bound
\(N>10^{1500}\) and strong multiplicity constraints \cite{OchemRao2012}.
...
\]
The branch \(q=5\), for example, means that \(5\mid N\) and \(3\nmid N\).  It
does not mean merely that \(5\mid N\).  This distinction is essential.  An odd
perfect number divisible by both \(3\) and \(5\), if it existed, would belong to
the minimal-prime branch \(q=3\), not to the branch \(q=5\).
...
no odd perfect number with
\[
  \min\{p:p\mid N\}\in\{5,7,11,13,17\}.
\]
...
complete proof program must still treat
\[
  q=3
  \qquad\text{and}\qquad
  q\ge19.
...
\[
  \CertRelease
\]
denotes the July 2026 release of the five small minimal-prime certificate
packages used in this paper.  It consists of the bundle files, verifier files,
expected terminal outputs, and SHA256 hashes listed in
Table~\ref{tab:reproducibility-release}.  The word ``release'' is meant
literally: the proof statements below are relative to these frozen files, not
to whatever local working copy may exist later.
For readability, after this definition we also write
\[
  \Csmall := \CertRelease.
\]
\end{definition}
...
no odd perfect number can be divisible by \(11\).  If an odd perfect number
were divisible by \(3\) and \(11\), it would lie in the \(q=3\) branch.
\end{remark}
...
\end{center}
The corresponding Git release tag is \texttt{C-small-2026-07}.
...
auditability, this section includes the fuller \(q=5\) derivation extracted
from the revised source draft.  This is the didactic branch: it shows in detail
how lower-prime avoidance, the first \(5\)-input split, second-level cofactors,
finite successor windows, and parametric tail records fit together before the
same forced-or-pure pattern is reused for \(q=7,11,13,17\).
...
\[
  \min\{p:p\mid N\}\in\{5,7,11,13,17\}.
\]
Equivalently, the certified branch inventories for
\[
  q=5,\ 7,\ 11,\ 13,\ 17
\]
are exhausted.
...
\begin{proof}
Theorem~\ref{thm:q5-closed} closes \(q=5\).  Theorem~\ref{thm:q7-closed}
closes \(q=7\).  Theorem~\ref{thm:q11-closed} closes \(q=11\).
Theorem~\ref{thm:q13-closed} closes \(q=13\).  Theorem~\ref{thm:q17-closed}
closes \(q=17\).  These are minimal-prime branch statements in the sense of
...
The closure theorem leaves two tasks outside the scope of this paper:
\[
  q=3
  \qquad\text{and}\qquad
  q\ge19.
