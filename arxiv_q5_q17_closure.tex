...
an exact tail-abundance product stays below \(2\).  The \(q=5\) closure is
presented as the detailed model; the closures for \(q=7,11,13,17\) use the same
forced-or-pure reduction and are reproduced from the included JSONL bundles by
the listed Python verifiers.
\end{abstract}
