...
\(q=5,7,11,13,17\): every branch generated from the exact \(q\)-adic valuation
balance for \(\sigma(N)=2N\) and the lower-prime avoidance constraints
terminates before a compatible branch realization can occur.  The valuation
...
In this work, these results serve only as branch filters and consistency
checks; the closures below are driven by valuation-labeled cyclotomic input and
finite certificates.
