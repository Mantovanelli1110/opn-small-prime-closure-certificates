...
\newcommand{\CertRelease}{\Csmall}
\newcommand{\sha}[1]{\nolinkurl{#1}}
\newcommand{\shasplit}[2]{\begin{tabular}[t]{@{}l@{}}{\ttfamily\tiny #1}\\[-1pt]{\ttfamily\tiny #2}\end{tabular}}
\urlstyle{same}

\title{Certified Closures of the Remaining Small Minimal-Prime Branches for Odd Perfect Numbers}
\author{
Marco Mantovanelli\\
...
\begin{abstract}
For an odd perfect number \(N\), write
\(q=\min\{p:p\mid N\}\) for its smallest prime divisor.  Gr{\"u}n's upper
bound for \(q\) in terms of \(\omega(N)\), combined with Nielsen's theorem
\(\omega(N)\ge 9\), already implies \(q<8\); hence the minimal-prime branches
\(q=11,13,17\) are ruled out by the literature.  This paper gives
certificate-based closures for the remaining small minimal-prime branches
\(q=5\) and \(q=7\).  These are least-prime-divisor assertions: they do not
rule out divisibility by \(5\) or \(7\) for a hypothetical odd perfect number
whose smallest prime divisor is \(3\).  In each of the two new branches, every
case generated from the exact \(q\)-adic valuation balance for \(\sigma(N)=2N\)
and the lower-prime avoidance constraints terminates before a compatible
branch realization can occur.  The valuation balance specifies the \(q\)-adic
contribution that must be supplied by cyclotomic factors \(\sigma(p^e)\), while
lower-prime avoidance forbids any support prime below \(q\) from appearing in
those factors.  At the terminal records, the certificates verify the remaining
arithmetic obstruction: a residual cofactor forces additional support primes, a
pure cyclotomic equation has only excluded solutions, or an exact
tail-abundance product stays below \(2\).  The \(q=5\) branch is presented as
the detailed audit model, the \(q=7\) branch is closed by the same
forced-or-pure mechanism, and the release also records comparison inventories
for the literature-closed branches \(q=11,13,17\).
\end{abstract}
...
amicable-number landscape, and Cai's monograph \cite{Cai2022} surveys perfect
numbers alongside Fibonacci sequences and related recurrences.  Euler proved
that any such \(N\) has the form
\[
...
nonexistence proof.  Let
\[
  p_1<p_2<\cdots<p_k
\]
be the distinct prime divisors of a hypothetical odd perfect number, so that
\(p_1=q\) and \(k=\omega(N)\).  Gr{\"u}n proved the general least-prime-divisor
bound
\[
  p_1 < \frac{2}{3}k+2
\]
\cite{Grun1952}.  Nielsen proved \(k\ge9\), and later developed related
Diophantine and upper-bound methods \cite{Nielsen2007,Nielsen2015}.  Combining
Gr{\"u}n's bound with \(k\ge9\) gives
\[
  p_1 < \frac{2}{3}\cdot 9+2=8.
\]
Consequently the literature already restricts the smallest prime divisor of an
odd perfect number to
\[
  p_1\in\{3,5,7\};
\]
in particular, \(11,13,17\) cannot occur as smallest prime divisors.

Other global bounds provide additional context.  Ochem and Rao proved the
landmark lower bound \(N>10^{1500}\) and strong multiplicity constraints
\cite{OchemRao2012}.  Goto--Ohno
proved \(p_k>10^8\), while Iannucci proved \(p_{k-1}>10^4\) and
\(p_{k-2}>100\) \cite{GotoOhno2008,Iannucci1999,Iannucci2000}.  In the opposite
direction, Zelinsky proved \(p_{k-1}<(2N)^{1/5}\) and
\(p_{k-1}p_k<6^{1/4}N^{1/2}\), and Bibby--Vyncke--Zelinsky proved
\(p_{k-2}<(2N)^{1/6}\) and \(p_{k-2}p_{k-1}p_k<(2N)^{3/5}\)
\cite{Zelinsky2019,BibbyVynckeZelinsky2021}.  For prime-factor counts, Zelinsky
and Clayton--Hansen sharpened inequalities relating the number \(\omega(N)\) of
distinct prime factors to the total number \(\Omega(N)\) counted with
multiplicity; for example Clayton--Hansen proved
\[
  \frac{99}{37}\omega(N)-\frac{187}{37}\le \Omega(N),
  \qquad
  3\nmid N\Rightarrow
  \frac{46}{19}\omega(N)-\frac{51}{19}\le \Omega(N)
\]
\cite{Zelinsky2021,ClaytonHansen2023}.  Zelinsky also obtained lower bounds for
the number of distinct prime factors in terms of the second, third, and fourth
smallest prime factors, Stone proved improved product bounds and
\(p_1<3k/7+3\) for the smallest prime divisor, and Zelinsky bounded the
reciprocal sum \(T(N)=\sum_{p\mid N}1/p\) and the product
\(H(N)=\prod_{p\mid N}p/(p-1)\) in terms of \(p_1\)
\cite{Zelinsky2023SmallPrimes,Stone2024,Zelinsky2025ReciprocalPrimeDivisors}.

The new part of the present paper begins after the Gr{\"u}n--Nielsen reduction.
The branches \(q=11,13,17\) are already closed by the preceding literature
corollary.  The remaining small branches are \(q=5\) and \(q=7\), and these are
the branches closed here by finite certificates.  This is different in kind
from a new lower bound for \(N\), for \(\omega(N)\), or for the largest prime
divisors, and it is not a divisibility exclusion such as ``\(5\nmid N\)''.  It
is an exact closure of least-prime-divisor branches.  Thus the \(q=5\) branch is
the case \(5\mid N\) and \(3\nmid N\); a hypothetical odd perfect number
divisible by both \(3\) and \(5\) would belong to the \(q=3\) branch, not to the
\(q=5\) branch.  The release also contains certificate inventories for
\(q=11,13,17\), but those inventories are included only for comparison,
reproducibility, and stress-testing of the certificate framework, not as the
source of the exclusion of those three branches.

We restrict attention to a finite list of minimal-prime branches.  Write
\(S=\{p:p\mid N\}\), \(q=\min S\), and
\(N=\prod_{p\in S}p^{e_p}\).  Thus the branch \(q=5\), for example, means that
\(5\mid N\) and \(3\nmid N\), not merely that \(5\mid N\).

The main new certificate theorem is the following.

\begin{theorem}[Certified closure of the remaining small branches]
...
no odd perfect number with \(\min\{p:p\mid N\}\in\{5,7\}\).
\end{theorem}

Together with the Gr{\"u}n--Nielsen reduction, this leaves only the branch
\(q=3\) for any hypothetical odd perfect number.  The theorem is intentionally
scoped: it is not an unconditional proof that odd perfect numbers do not exist.
The branch \(q=3\) has no lower-prime avoidance layer and therefore behaves
differently from the branches treated here.

The contribution of this paper is threefold.  First, it isolates the
minimal-prime branch problem from the full odd-perfect-number problem and
places it after the Gr{\"u}n--Nielsen least-prime-divisor reduction.  Second, it
gives a uniform valuation-and-avoidance mechanism that certifies the remaining
small branches \(q=5,7\).  Third, it records a frozen, reproducible certificate
release whose primary verifiers exhaust the \(q=5\) and \(q=7\) inventories,
with comparison inventories for the literature-closed branches \(q=11,13,17\).
...
Let \(\Csmall\) denote the immutable certificate release with release identifier
\texttt{C-small-2026-07}.  It consists of the bundle files, verifier files,
expected terminal outputs, and SHA256 hashes listed in the reproducibility
table in Section~\ref{sec:certificates}.  The word ``release'' is meant
literally: the proof statements below are relative to these frozen files, not
to whatever local working copy may exist later.  The subscript ``small'' refers
to the small minimal-prime branch package: the primary branches \(q=5,7\) and
the comparison inventories for the literature-closed branches \(q=11,13,17\).
\end{definition}

\begin{definition}[Release branch inventory]
\label{def:release-branch-inventory}
For \(q=5,7,11,13,17\), the \emph{\(\Csmall\)-branch inventory} is the finite,
machine-readable collection of coverage splits, forced cofactors, leaf-status
records, tail certificates, pure-exception records, and master coverage wrappers
listed in the release JSONL certificate files.  The \(q=5\) and \(q=7\)
inventories are the primary inventories used for the new closure theorem.  The
\(q=11,13,17\) inventories are comparison inventories: the corresponding
minimal-prime branches are already excluded by the Gr{\"u}n--Nielsen reduction,
but the release records them to make the certificate framework reproducible
across the same small-prime range.  A branch inventory is called \emph{exhausted
relative to \(\Csmall\)} when every leaf in that inventory is certified as
refuted, forced-tail-controlled, pure-tail-controlled, or delegated to a
certified child archive, and when the master wrapper verifies that the listed
leaves cover the parent branch.
\end{definition}

\begin{remark}[Scope warning]
\label{rem:scope-warning}
Every closure theorem below is a minimal-prime theorem.  The statement
``the \(q=7\) branch is closed'' means
\[
  \min\{p:p\mid N\}=7
\]
has no realization in the certified branch inventory.  It does not assert that
no odd perfect number can be divisible by \(7\); it excludes only the case in
which \(7\) is the smallest prime divisor.  The same convention applies to the
comparison inventories for \(q=11,13,17\), although those branches are already
excluded by the Gr{\"u}n--Nielsen literature bound.
\end{remark}
...
\begin{corollary}[Support floors used by the certificates]
\label{cor:support-floors}
In the primary branches \(q=5,7\), and in the comparison inventories
\(q=11,13,17\), the active published lower bound for the number of distinct
prime divisors used by the certificates is
\[
\begin{array}{c|c}
...
which gives \(\omega(N)\ge27\) in this situation.  The latter row is used only
inside the comparison certificate inventories, since \(q=11,13,17\) are already
excluded as smallest prime divisors by the Gr{\"u}n--Nielsen reduction.
\end{proof}
...
\begin{proposition}[Coverage split for the small certificate inventories]
...
Table~\ref{tab:first-input-labels}.  For \(q=11,13,17\) this is used only to
describe the comparison inventories, since those branches are already
literature-closed by Gr{\"u}n--Nielsen.
\end{proposition}
...
\(r\in S\).  Moreover, if \(r\mid\Phi_d(x)\) and \(r\nmid d\), then
\(\ord_r(x)=d\).
\end{lemma}
...
\midrule
\(q=5\) &
parametric \(C_5(x)\)-tail, pure \(C_2(\pi)=2\) tail, and \(C_2(\pi)\)-post-window records\\
\(q=7\) &
...
For auditability, the division of labour is deliberately narrow.  The
handwritten proof establishes Euler-form reductions, the \(q\)-adic valuation
balance, lower-prime avoidance, the forced-prime alternative, the tail-envelope
lemma, and the final implication from certified leaf exhaustion to branch
closure.  The machine layer checks only the frozen finite data: coverage labels,
factorizations, order and valuation records, the pure Diophantine screen
records in Table~\ref{tab:pure-screen-details}, the tail-count records
including the release value \(M=18\), and the exact rational endpoint and tail
inequalities required by the checker contract in Criterion~\ref{crit:master-wrapper}.
In particular, the scripts do not discover new branches during verification;
they verify that the listed frozen branch inventories are complete and that
each listed terminal leaf has an accepted certificate reason for rejection or
tail control.
...
record carries a tail-count certificate with \(B=59\) and \(M=18\), and the
master wrapper accepts the final tail inequality only after that record has
been checked.  The proof does not require \(M=18\) to be optimal.
\end{definition}
...
\shasplit{95b536eb2ed411df8b229bfcb133a2d2}{ee5eb8d503c614e0c5c3b7edd159f867} &
...
\bottomrule
\end{tabularx}
\caption{Reproducibility table for the frozen certificate release \(\Csmall\).}
...
Table~\ref{tab:reproducibility-release} lists only the entry-point bundles and
verifier scripts used in the proof.  The complete frozen file list, including
imported dependency modules, is recorded in \path{RELEASE_MANIFEST.tsv}; its
hashes are duplicated in \path{SHA256SUMS.txt}.
\end{remark}
...
\]
is an integer \(>1\).  If \(C_2(\pi)=2\), the row is the pure family
\(\pi=2\cdot5^b-1\) and is delegated to the pure \(C_2\)-tail certificate.  If
\(3\mid C_2(\pi)\), the row is refuted by lower-prime avoidance.  Otherwise
\(C_2(\pi)\) has an odd prime divisor and forces a support prime \(r>5\) with
\[
  \ord_r(\pi)=2.
\]
\end{enumerate}
...
Since \(\pi\equiv1\pmod4\), the quotient has exactly one factor \(2\).  Thus
the case \(C_2(\pi)=2\) is precisely the pure family
\(\pi=2\cdot5^b-1\).  Outside this pure case, a quotient not divisible by \(3\)
has a surviving odd prime divisor \(r>5\), and this divisor of \(\pi+1\) has
\(\ord_r(\pi)=2\).
\end{proof}
...
\]
together with parametric post-window records and the pure
\(C_2(\pi)=2\) tail certificate at the endpoint \(\pi=1249\).  It closes
\(\mathbf{Q5E1}\) by
...
\section{Audit Trail for the \(q=5\) Closure}
...
used for the \(q=7\) closure and mirrored in the comparison inventories for
\(q=11,13,17\).
...
\input{arxiv_q5_expanded_detail.tex}
\endgroup

The \(q=7\) branch is shorter because the same mechanism has already been
displayed in detail for \(q=5\).  For \(q=11,13,17\), the subsequent sections
record only comparison frontier layers: these branches are already
literature-closed, but the certificate release includes their inventories for
...
\section{Comparison Certificate for the Literature-Closed \(q=11\) Branch}
\label{sec:q11}

The branch \(q=11\) is already excluded by the Gr{\"u}n--Nielsen reduction.
The release nevertheless records a comparison certificate for this branch.  For
that certificate, lower-prime avoidance applies to \(3,5,7\), and the active
...
\begin{theorem}[Comparison exhaustion of the \(q=11\) inventory]
...
\]
Therefore all children in the complete \(q=11\) comparison split are certified
as exhausted.  The exclusion of \(q=11\) as an actual minimal-prime branch does
not rely on this comparison certificate; it already follows from
Gr{\"u}n--Nielsen.
\end{proof}

\section{Comparison Certificate for the Literature-Closed \(q=13\) Branch}
\label{sec:q13}

The branch \(q=13\) is also literature-closed by Gr{\"u}n--Nielsen.  The
\(\Csmall\) release includes the following comparison inventory.  For \(q=13\),
lower-prime avoidance applies to \(3,5,7,11\).  The first-input coverage split
has the six children
\[
...
\begin{theorem}[Comparison exhaustion of the \(q=13\) inventory]
...
\]
Hence every child of the complete \(q=13\) comparison coverage split is
certified as exhausted.
\end{proof}

\section{Comparison Certificate for the Literature-Closed \(q=17\) Branch}
\label{sec:q17}

The branch \(q=17\) is likewise already excluded as a smallest-prime branch by
the Gr{\"u}n--Nielsen reduction.  The release includes a smaller comparison
inventory for it.  For \(q=17\), lower-prime avoidance applies to
\[
  3,\ 5,\ 7,\ 11,\ 13.
\]
...
\begin{theorem}[Comparison exhaustion of the \(q=17\) inventory]
...
\]
Thus every child in the complete \(q=17\) comparison split is certified as
exhausted.
\end{proof}
...
\begin{theorem}[Certified closure for \(q=5,7\)]
...
\(\min\{p:p\mid N\}\in\{5,7\}\).  Equivalently, the primary certified branch
inventories for \(q=5\) and \(q=7\) are exhausted.
\end{theorem}

\begin{proof}
Theorem~\ref{thm:q5-closed}, equivalently the detailed
Theorem~\ref{thm:q5-current-branch-inventory-closed}, closes \(q=5\).
Theorem~\ref{thm:q7-closed} closes \(q=7\).  These are minimal-prime branch
statements in the sense of
...
also divisible by a smaller prime.  Combining the two primary branch theorems
gives the displayed final statement.
\end{proof}

\begin{corollary}[Remaining minimal-prime branch]
\label{cor:remaining-q3}
Assume the Gr{\"u}n--Nielsen least-prime-divisor reduction and the certified
closure theorem above.  If an odd perfect number exists, then its smallest
prime divisor is \(3\).
\end{corollary}

\begin{proof}
By Gr{\"u}n's bound \(p_1<2\omega(N)/3+2\) and Nielsen's theorem
\(\omega(N)\ge9\), every odd perfect number has
\[
  p_1<8.
\]
Thus \(p_1\in\{3,5,7\}\).  Theorem~\ref{thm:final-closure} excludes
\(p_1=5\) and \(p_1=7\), leaving only \(p_1=3\).
\end{proof}

\begin{remark}[Remaining branches]
The only remaining minimal-prime branch is \(q=3\).  This branch has no
lower-prime avoidance constraints and therefore lies outside the method used
for \(q=5\) and \(q=7\).
\end{remark}

\section*{Acknowledgements}

I thank Beno\^{\i}t Clo\^{\i}tre for helpful feedback on an earlier draft of
this paper.
