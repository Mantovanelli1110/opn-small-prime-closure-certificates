...
\[
  \min S=5,
\]
namely the branch in which the Euler prime is congruent to \(2\) modulo \(3\).
...
\[
  \tau(\mathcal B_{5,\pi\equiv 2(3)})=\mathsf{small}(5),
\]
so that
\[
  \min S=5.
\]
The branch also records that the Euler prime $\pi$ satisfies
\[
  \pi\equiv 2\pmod 3
\]
and that its exponent $\alpha$ satisfies the Euler schema
\[
...
\]
has no realization by an odd perfect number.  Equivalently, if
\[
  \min S=5,
\]
then the Euler prime must satisfy
\[
  \pi\equiv 1\pmod 3.
\]
\end{proposition}
...
\[
  \min S=5,
\]
the prime $3$ is not in the support:
\[
  3\notin S.
\]
Therefore lower-prime avoidance with $\ell=3$ applies to every prime power in
the support.  In particular,
\[
  \operatorname{Avoid}_3(\pi,\alpha)
\]
must hold.

But the branch assumes
\[
  \pi\equiv2\pmod3.
\]
Hence
\[
  \ord_3(\pi)=2.
\]
Since $\pi$ is the Euler prime, its exponent satisfies
\[
  \alpha\equiv1\pmod4.
\]
Thus
...
\[
  2\mid \alpha+1.
\]
By the cyclotomic input condition, this gives
\[
  3\mid \sigma(\pi^\alpha).
\]
Equivalently,
\[
  v_3(\sigma(\pi^\alpha))>0.
\]
This contradicts lower-prime avoidance, which requires
\[
  v_3(\sigma(\pi^\alpha))=0.
\]
...
If $N$ is an odd perfect number with
\[
  \min S=5,
\]
then $5$ is not the Euler prime.  Consequently
\[
  e_5\equiv0\pmod2.
\]
\end{corollary}
...
\[
  ...
  \pi\equiv2\pmod3
\]
holds.  Proposition~\ref{prop:q5-euler-2mod3-empty} refutes the second case.
\end{proof}
...
\[
  \min S=5.
\]
Let $\pi$ be the Euler prime, with Euler exponent $\alpha$.

Then the following constraints hold.

\begin{enumerate}[label=(\roman*),leftmargin=2.2em]

\item The Euler prime satisfies
      \[
        \pi\equiv1\pmod3,
        \qquad
        \alpha\equiv1\pmod4,
...
\item Let $p\in S$, $p\ne5$, be non-Euler.  Then
      ...
      \[
        \begin{cases}
          3\nmid e_p+1, & p\equiv1\pmod3,\\[4pt]
          \text{automatic}, & p\equiv2\pmod3.
        \end{cases}
      \]
...
\item The Euler prime $\pi$ is a $5$-input prime precisely in the following two
      cases:
      \[
        \pi\equiv1\pmod5
        \qquad\text{and}\qquad
...
\item If
      ...
      \[
        4\nmid \alpha+1,
      \]
      so such an Euler prime is $5$-neutral.
\end{enumerate}
\end{lemma}
...
\[
  \ord_3(\pi)=1.
\]
Lower-prime avoidance for $\ell=3$ therefore gives
\[
  3\nmid\alpha+1.
\]
The congruence
...
non-Euler $5$-input can only have
\[
  \ord_5(p)=1,
\]
i.e.
...
\]
More explicitly, every odd perfect number with
\[
  \min S=5
\]
realizes at least one of the following child branches:
...
The complementary child branch, in which every prime
\[
  p\in S,\qquad p\ne5,
\]
is $5$-neutral, is empty.
\end{corollary}
...
witness is either of order one modulo $5$, giving
\[
  \mathbf{Q5E1},
\]
or of order two modulo $5$, giving
\[
  \mathbf{Q5E2}.
\]
...
types:
\[
  \mathtt{Q5N},\qquad \mathtt{Q5E1},\qquad \mathtt{Q5E2}.
\]
...
\mathbf{Q5N}:
\qquad
\text{there exists a non-Euler prime }p\in S
\text{ such that }
p\ne5,\ p\ne\pi,
\]
\[
...
\[
  ...
  \mathbf{Q5N}
\]
form a complete coverage split of the \(q=5\) minimal-prime branch.  The split is
a coverage split, not necessarily a disjoint split.
\end{proposition}
...
\[
  e_5>0.
\]
The \(5\)-adic valuation balance for
\[
  \sigma(N)=2N
\]
is
...
one summand is positive.  Hence there exists a prime
\[
  p\in S,
  \qquad
  p\ne5,
...
\[
  v_5(\sigma(p^{e_p}))>0.
\]
We call such a prime a \(5\)-input witness.
...
\[
  \operatorname{ord}_5(p)\mid e_p+1
\]
if \(\operatorname{ord}_5(p)>1\), or
\[
  5\mid e_p+1
\]
if \(\operatorname{ord}_5(p)=1\).
...
\[
  \operatorname{ord}_5(p)=1.
\]
Equivalently,
\[
  p\equiv1\pmod5.
\]
The order-one input condition then gives
\[
  5\mid e_p+1.
\]
This is precisely the branch
\[
  \mathbf{Q5N}.
\]
...
Since \(\pi\) is a \(5\)-input witness,
\[
  v_5(\sigma(\pi^\alpha))>0.
\]
Let
\[
...
\[
  \pi\equiv1\pmod5,
\]
and the order-one cyclotomic input condition gives
\[
  5\mid\alpha+1.
\]
This is exactly the branch
\[
  \mathbf{Q5E1}.
\]
...
\[
  d=4,
\]
then a \(5\)-adic input from \(\sigma(\pi^\alpha)\) would require
\[
  4\mid \alpha+1.
\]
But
\[
  \alpha+1\equiv2\pmod4,
\]
so this is impossible.  Hence the Euler witness cannot have order \(4\) modulo
\(5\).

Thus every possible \(5\)-input witness leads to one of
\[
  \mathbf{Q5N},
  \qquad
  \mathbf{Q5E1},
  ...
  \mathbf{Q5E2}.
\]
Therefore the three displayed children cover all realizations of the branch
\[
  \min\{p:p\mid N\}=5.
\]
...
branches
\[
  \mathbf{Q5N},\qquad \mathbf{Q5E1},\qquad \mathbf{Q5E2}
\]
from Corollary~\ref{cor:q5-first-input-coverage-split}.
...
\[
  p\in S,\qquad p\ne5,\qquad p\ne\pi,
\]
be a non-Euler $5$-input witness in the branch $\mathbf{Q5N}$.  Thus
\[
  p\equiv1\pmod5
  \qquad\text{and}\qquad
  5\mid e_p+1.
...
\[
  r>5,
\]
and every such divisor is forced into the support:
\[
  r\in S.
\]
Moreover, for every prime divisor
\[
  r\mid C_5(p)
\]
with \(r>5\), one has
...
\[
  \Phi_5(p)>5
\]
for every prime \(p\ge7\), and in the branch \(\min S=5\) we have
\[
  p\ne5.
\]
Hence
...
be prime.  Then
\[
  r\mid \Phi_5(p)\mid\sigma(p^{e_p})\mid\sigma(N)=2N.
\]
Since \(r\) is odd, this gives
\[
  r\mid N,
\]
so
...
If \(r=3\), this contradicts
\[
  \min S=5,
\]
because \(3\notin S\).  Hence any surviving branch has a prime divisor
\[
  r>5
\]
of \(C_5(p)\), and such an \(r\) is forced into \(S\).
...
because
\[
  \Phi_5(p)=p^4+p^3+p^2+p+1\equiv1\pmod p.
\]
Since
\[
  r\mid\Phi_5(p),
\]
the exact order of \(p\) modulo \(r\) is
\[
  \ord_r(p)=5.
\]
Therefore
...
replaced by the Euler prime \(\pi\).  Since
\[
  5\mid\alpha+1,
\]
the factor
...
\[
  5^c\equiv1\pmod4
\]
for every \(c\ge1\).  This is impossible.  Hence no such \(\pi\) and \(c\) exist.
\end{proof}
...
\[
  2\mid \alpha+1.
\]
Therefore the cyclotomic factor
\[
  \Phi_2(\pi)=\pi+1
\]
divides
...
\[
  \pi+1=5^{c_\pi},
\]
contradicting Lemma~\ref{lem:no-pure-q5e2-exception}.  Hence
\[
  C_2(\pi)>1.
\]
...
be prime.  Then
\[
  r\mid \pi+1\mid\sigma(\pi^\alpha)\mid\sigma(N)=2N.
\]
Since \(r\) is odd, this gives
\[
  r\mid N,
...
\[
  \min S=5.
\]
Thus every surviving branch has a prime divisor
\[
  r>5
\]
of \(C_2(\pi)\), and such an \(r\) is forced into \(S\).

For every such \(r\), one has
\[
  \pi\equiv-1\pmod r.
\]
Since \(r>5\), in particular \(r\ne2\), and therefore
\[
  \ord_r(\pi)=2.
\]
\end{proof}
...
branch inventory.  The child closures are proved in the following order:
\[
  \mathbf{Q5E2},
  \qquad
  \mathbf{Q5E1},
...
We first close the order-two Euler-input child branch 
\[ 
\mathbf{Q5E2}:\qquad \pi\equiv-1\pmod5. 
\] 
...
\[ 
(109,11), \qquad (229,23), \qquad (349,7), \qquad (409,41). 
\] 
...
Work in the \(\mathbf{Q5E2}\) successor branch with
\[
  (\pi,r)=(349,7).
\]
Let \(e_7\ge94\) be even and put
\[
  n:=e_7+1.
\]
Then the corresponding post-window row is closed.

More precisely, if
\[
  3\mid n,
\]
then the row is refuted by lower-prime avoidance, since
\[
  \operatorname{ord}_3(7)=1
\]
and hence
\[
  3\mid \sigma(7^{e_7}).
...
\[
  3\nmid n,
\]
then Zsigmondy's theorem applied to \(7^n-1\) gives a primitive prime divisor
\[
  s\mid 7^n-1
\]
with
\[
  \operatorname{ord}_s(7)=n.
\]
Thus
\[
  s\mid \Phi_n(7)\mid\sigma(7^{e_7}),
\]
so \(s\) is forced into the support.  Since
\[
  n\mid s-1
\]
...
\]
The row is consequently tail-controlled with
\[
  B=59,
  \qquad
  M=18.
...
\begin{proof}
The case \(3\mid n\) follows from lower-prime avoidance.  Since \(7\equiv1\pmod3\),
one has \(\operatorname{ord}_3(7)=1\), so \(3\mid n=e_7+1\) implies
\[
  3\mid\sigma(7^{e_7}),
\]
contradicting \(3\notin S\).

Assume \(3\nmid n\).  Since \(e_7\) is even, \(n\) is odd.  Also \(n\ge95\), so
Zsigmondy's exceptional cases for \(7^n-1\) do not apply.  Thus \(7^n-1\) has a
primitive prime divisor \(s\).  By primitivity,
\[
  \operatorname{ord}_s(7)=n,
\]
so \(s\mid\Phi_n(7)\).  Since \(\Phi_n(7)\) divides \(\sigma(7^{e_7})\), the
prime \(s\) is forced into the support.

The order relation gives
\[
  n\mid s-1.
\]
Because \(n\) is odd and \(s\) is odd, \(s-1\) is an even multiple of \(n\), so
\[
  s\ge2n+1.
\]
For \(n\ge95\), this gives
\[
  s\ge191.
\]
...
\[
  n:=e_{11}+1.
\]
Then the corresponding post-window row is tail-controlled.

More precisely, since \(e_{11}\) is even, \(n\) is odd.  The divisor-sum factor
is
\[
  \sigma(11^{e_{11}})
  =
...
\]
By Zsigmondy's theorem, applied to \(11^n-1\), there exists a primitive prime
divisor
\[
  s\mid 11^n-1
\]
with
\[
  \operatorname{ord}_s(11)=n.
\]
Equivalently,
\[
  s\mid \Phi_n(11).
\]
Hence \(s\) is forced into the support.
...
\[
  n\mid s-1.
\]
Moreover \(n\) is odd and \(s\) is odd, so \(s-1\) is an even multiple of \(n\).
Therefore
\[
  s\ge2n+1.
\]
...
\[
  \operatorname{ord}_s(11)=n,
\]
so \(s\mid\Phi_n(11)\).  Since \(\Phi_n(11)\mid\sigma(11^{e_{11}})\), this
prime \(s\) is forced into the support.
...
More precisely, since \(e_{23}\) is even, the integer \(n\) is odd.  The
divisor-sum factor is
\[
  \sigma(23^{e_{23}})
  =
  \frac{23^n-1}{23-1}
...
\]
By Zsigmondy's theorem, applied to \(23^n-1\), there exists a primitive prime
divisor
\[
  s\mid 23^n-1
\]
with
\[
  \operatorname{ord}_s(23)=n.
\]
Equivalently,
\[
  s\mid \Phi_n(23).
\]
Thus \(s\) divides \(\sigma(23^{e_{23}})\), and hence \(s\) is forced into the
support.
...
\[
  n\mid s-1.
\]
Moreover \(n\) is odd and \(s\) is odd, so \(s-1\) is an even multiple of
\(n\).  Therefore
\[
  s\ge2n+1.
\]
...
\(\Phi_n(23)\).  Consequently every even post-window row
\[
  e_{23}\ge22
\]
is tail-controlled with
...
\[
  n=e_{23}+1.
\]
Since \(e_{23}\) is even, \(n\) is odd.  Also \(e_{23}\ge22\), so
\[
  n\ge23.
\]
Thus the exceptional cases in Zsigmondy's theorem do not apply to
\[
  23^n-1.
\]
Hence there exists a primitive prime divisor \(s\) of \(23^n-1\).  By
primitivity,
\[
  \operatorname{ord}_s(23)=n.
\]
Therefore \(s\) divides the primitive cyclotomic factor
\[
  \Phi_n(23).
\]
Since
\[
  \Phi_n(23)\mid \sigma(23^{e_{23}}),
\]
the prime \(s\) is forced into the support of any realization of the branch.

The order relation gives
\[
  n\mid s-1.
\]
Because \(n\) is odd and \(s\) is odd, \(s-1\) is an even multiple of \(n\).
Thus
\[
  s\ge2n+1.
\]
...
More precisely, since \(e_{41}\) is even, the integer \(n\) is odd.  The
divisor-sum factor is
\[
  \sigma(41^{e_{41}})
  =
...
\[
  \Phi_n(41).
\]
By Zsigmondy's theorem, applied to \(41^n-1\), there exists a primitive prime
divisor
\[
  s\mid 41^n-1
\]
with
\[
  \operatorname{ord}_s(41)=n.
\]
Equivalently,
\[
  s\mid \Phi_n(41).
\]
Thus \(s\) divides \(\sigma(41^{e_{41}})\), and hence \(s\) is forced into the
support.
...
\(\Phi_n(41)\).  Consequently every even post-window row
\[
  e_{41}\ge22
\]
is tail-controlled with
...
\[
  n=e_{41}+1.
\]
Since \(e_{41}\) is even, \(n\) is odd.  Also \(e_{41}\ge22\), so
\[
  n\ge23.
\]
Thus the exceptional cases in Zsigmondy's theorem do not apply to
\[
  41^n-1.
\]
Hence there exists a primitive prime divisor \(s\) of \(41^n-1\).  By
primitivity,
\[
  \operatorname{ord}_s(41)=n.
\]
...
\[
  (349,7)
\]
is closed by finite verification of the rows
\[
  22\le e_7\le92,
  \qquad
  e_7\equiv0\pmod2,
\]
together with the parametric Zsigmondy--tail-control criterion for all even
\[
  e_7\ge94.
\]
...
\[
  (109,11)
\]
is closed by the aggregation-safe \(E40\) successor archive and by the
parametric post-window criterion for all even
\[
  e_{11}\ge22.
\]
...
\[
  (229,23)
\]
is closed in the same way.  Its finite \(E40\) successor archive is
aggregation-safe, and its parametric certificate covers every even
\[
  e_{23}\ge22.
\]
...
\[
  (409,41)
\]
is also closed by an aggregation-safe \(E40\) successor archive and a
parametric certificate for all even
\[
  e_{41}\ge22.
\]
...
corresponding closures of
\[
  \mathbf{Q5E1}
  \qquad\text{and}\qquad
  \mathbf{Q5N}
\]
and with the certified first \(5\)-input coverage split.
\end{remark}
...
\[
  ...
  5\mid\alpha+1.
\]
In the surviving \(q=5\) branch the Euler prime also satisfies
\[
  \pi\equiv1\pmod3
  \qquad\text{and}\qquad
  \pi\equiv1\pmod4.
\]
Hence every \(\mathbf{Q5E1}\) Euler-prime candidate satisfies
\[
  \pi\equiv1\pmod{60}.
\]
The finite exceptional window
\[
  \pi<1381,
  \qquad
  \pi\equiv1\pmod{60},
\]
is exhausted by the frozen finite archive, while all remaining rows
\[
  \pi\ge1381,
  \qquad
  \pi\equiv1\pmod{60},
\]
are closed by the parametric \(C_5(\pi)\)-tail certificate below.
...
\[
  \mathbf{Q5E1}.
\]
Thus the Euler prime satisfies
\[
  \pi\equiv1\pmod5,
  \qquad
  5\mid \alpha+1.
\]
Together with the surviving \(q=5\) constraints
\[
  \pi\equiv1\pmod3,
  \qquad
  \pi\equiv1\pmod4,
...
\[
  \pi\ge1381.
\]
Then the \(\mathbf{Q5E1}\) row attached to \(\pi\) is tail-controlled.
...
\[
  v_5(\Phi_5(\pi))=1,
\]
so \(C_5(\pi)\) is an integer.  Moreover,
\[
  C_5(\pi)>1.
\]
Any prime divisor
\[
  r\mid C_5(\pi)
\]
is forced into the support.  The primes \(3\) and \(5\) do not divide
\(C_5(\pi)\), and every remaining prime divisor satisfies
\[
  r>5,
  \qquad
  \operatorname{ord}_r(\pi)=5,
  \qquad
  r\equiv1\pmod5.
\]
Hence
...
The verifier checks the worst-case tail-control inequality at
\[
  \pi=1381,
  \qquad
  r=11.
...
\[
  H(\{5,\pi,r\})
\]
is decreasing in both \(\pi\) and \(r\), this endpoint check implies
\[
  H(\{5,\pi,r\})
  \left(\frac{59}{58}\right)^{18}
  <2
...
\[
  ...
  r\mid C_5(\pi).
\]
Thus every \(\mathbf{Q5E1}\) row with \(\pi\ge1381\) is tail-controlled with
\[
  B=59,
  \qquad
  M=18.
...
\[
  \pi\equiv1\pmod{60}
\]
follows from the \(\mathbf{Q5E1}\) branch conditions together with the surviving
\(q=5\) Euler-prime constraints.

Because \(5\mid\alpha+1\), the cyclotomic factor
\[
  \Phi_5(\pi)
\]
divides
\[
  \sigma(\pi^\alpha).
\]
Since \(\pi\equiv1\pmod5\), the standard LTE calculation gives
\[
  v_5(\Phi_5(\pi))=1.
\]
Thus
\[
  C_5(\pi)=\Phi_5(\pi)/5
\]
is an integer.  It is greater than \(1\) for every \(\pi\ge61\), hence in
particular for every \(\pi\ge1381\).
...
be prime.  Then
\[
  r\mid \Phi_5(\pi)\mid\sigma(\pi^\alpha)\mid\sigma(N)=2N.
\]
Since \(r\) is odd, it follows that
\[
  r\mid N,
\]
so \(r\) is forced into the support.
...
\[
  3\nmid C_5(\pi).
\]
Thus every prime divisor \(r\) of \(C_5(\pi)\) satisfies
\[
  r>5.
\]
...
\[
  r\equiv1\pmod5.
\]
The smallest possible such prime is
\[
  r=11.
\]
...
\[
  ...
  \pi\equiv1\pmod{60},
\]
is exhausted by direct enumeration of the corresponding prime candidates:
\[
  61,181,241,421,541,601,661,1021,1201,1321.
\]
The frozen package
\[
  \mathcal D_{5,E1}^{(1)}
\]
certifies all ten leaves as
\[
  \mathtt{child\_archive\_tail\_controlled}.
\]

For every remaining candidate, one has
\[
  \pi\ge1381,
  \qquad
  \pi\equiv1\pmod{60}.
\]
The parametric criterion applies and supplies a forced support prime
\[
  r\mid C_5(\pi)
\]
with
...
Together with the corresponding closures of
\[
  \mathbf{Q5E2}
  \qquad\text{and}\qquad
  \mathbf{Q5N},
\]
and with the first \(5\)-input coverage split, it contributes to the final
closure of the \(\CertRelease\) \(q=5\) branch inventory.
\end{remark}
...
\[
  \mathbf{Q5N}.
\]
In this child branch there is a non-Euler witness prime
\[
  p\in S,
  \qquad
  p\ne5,
...
\[
  ...
  e_p\equiv0\pmod2.
\]
The finite exceptional witness window
\[
  p<211,
  \qquad
  p\equiv1\pmod5,
...
\[
  ...
  p\equiv1\pmod5,
\]
are closed by the parametric \(C_5(p)\)-tail certificate below.
...
\[
  \mathbf{Q5N}.
\]
Thus there is a non-Euler \(5\)-input witness
\[
  p\in S,
  \qquad
  p\ne5,
...
\[
  p\ge211.
\]
Then the \(\mathbf{Q5N}\) witness row attached to \(p\) is tail-controlled.
...
\[
  v_5(\Phi_5(p))=1,
\]
so \(C_5(p)\) is an integer.  Moreover,
\[
  C_5(p)>1.
\]
...
\[
  H(\{5,p,r\})
\]
is decreasing in both \(p\) and \(r\), this endpoint check implies
\[
  H(\{5,p,r\})
  \left(\frac{59}{58}\right)^{18}
  <2
...
\[
  ...
  r\mid C_5(p).
\]
Thus every \(\mathbf{Q5N}\) witness row with \(p\ge211\) is tail-controlled with
\[
  B=59,
  \qquad
  M=18.
...
\begin{proof}
Because \(5\mid e_p+1\), the cyclotomic factor
\[
  \Phi_5(p)
\]
divides
\[
  \sigma(p^{e_p}).
\]
Since \(p\equiv1\pmod5\), the standard LTE calculation gives
\[
  v_5(\Phi_5(p))=1.
\]
Thus
...
\(\mathbf{Q5N}\) witness row has
\[
  3\nmid C_5(p).
\]
Hence every surviving prime divisor \(r\) of \(C_5(p)\) satisfies
\[
  r>5.
\]
...
The possible Euler-role split
\[
  r=\pi
  \qquad\text{or}\qquad
  r\ne\pi
\]
does not affect the tail-control inequality, since the verified kernel
\[
  \{5,p,r\}
\]
is already sufficient in either role.  Combining the finite exceptional window
...
\[
  ...
  \mathbf{Q5E1},
\]
and with the first \(5\)-input coverage split, this completes the three child
branches of the \(\CertRelease\) \(q=5\) branch inventory.
\end{remark}
...
\[
  \min\{p:p\mid N\}=5.
\]
The first necessary \(5\)-input coverage split gives the three branch families
\[
  \mathbf{Q5E2},
  \qquad
  \mathbf{Q5E1},
  ...
  \mathbf{Q5N}.
\]
This split is complete: every realization of the \(q=5\) branch realizes at
least one of these three children.

The \(\CertRelease\) \(\mathbf{Q5E2}\) successor inventory is exhausted by the finite
post-window closures for
\[
  (109,11),
  \qquad
  (229,23),
...
The \(\CertRelease\) \(\mathbf{Q5E1}\) successor inventory is exhausted by the finite
exceptional Euler-prime window
\[
  \pi<1381,
  \qquad
  \pi\equiv1\pmod{60},
\]
together with the parametric Euler-prime window closure for
\[
  \pi\ge1381.
\]

The \(\CertRelease\) \(\mathbf{Q5N}\) successor inventory is exhausted by the finite
exceptional witness-prime window
\[
  p<211,
  \qquad
  p\equiv1\pmod5,
\]
together with the parametric witness-prime window closure for
\[
  p\ge211.
\]
...
minimal-prime branch
\[
  q=\min\{p:p\mid N\}=5.
\]
It therefore excludes odd perfect numbers whose smallest prime divisor is \(5\)
inside the certificate release \(\CertRelease\).

It should not be read as the statement that no odd perfect number is divisible
by \(5\).  An odd perfect number divisible by \(5\) could also lie in the
branch
\[
  q=3,
\]
if it were divisible by both \(3\) and \(5\).  Such a number is not part of the
\(q=5\) minimal-prime branch.
\end{remark}
